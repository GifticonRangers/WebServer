...
public class NfcApiController {
    ...
    @Operation(summary = "auth NFC tag by Student",
            description = "NFC 인증\n\n"
                        + "weekAttendance(출석 주차), timeAttendance(출석 차시), idSubject(과목 기본키 값) 필요")
    @PostMapping("/api/nfc/authNFC/{id}")
    public ResponseEntity authNfc(@PathVariable Long id, @RequestBody AttendanceDTO.showAttendanceForm dto) {
        boolean state = nfcService.authNfc(id, dto);
